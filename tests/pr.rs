...
fn setup_mock_gh(test_name: &str) -> PathBuf {
    let mock_dir = PathBuf::from("./test_sandbox").join(test_name).join("mock_bin");
...
fn test_pr_command_creates_prs_for_chain() {
    ...
    assert!(stdout.contains("Pushed branch feature-1"), 
    ...
    assert!(stdout.contains("✅ Created PR for feature-2 -> feature-1"), 
            "Should show success message for feature-2, got: {}", stdout);
...
fn test_pr_command_skips_existing_prs() {
    ...
    // Assertions
    assert!(output.status.success(), "Command should succeed");
    assert!(stdout.contains("🔗 Open PR already exists for branch feature-with-pr"), 
            "Should skip existing PR for feature-with-pr, got: {}", stdout);
    assert!(stdout.contains("https://github.com/test/repo/pull/123"), 
            "Should show PR URL for feature-with-pr, got: {}", stdout);
    assert!(stdout.contains("🔗 Open PR already exists for branch feature-merged"), 
            "Should skip existing PR for feature-merged, got: {}", stdout);
    assert!(stdout.contains("https://github.com/test/repo/pull/124"), 
            "Should show PR URL for feature-merged, got: {}", stdout);
...
fn test_gh_cli_not_installed() {
    ...
    assert!(stderr.contains("GitHub CLI (gh) is not installed") || 
            ...
            "Should show error about gh not being installed, got: {}", stderr);
...
fn test_list_command_with_pr_flag() {
    ...
    assert!(stdout.contains("feature-with-pr"), "Should show branch name");
    assert!(stdout.contains("https://github.com/test/repo/pull/123"), 
            "Should show PR URL for feature-with-pr, got: {}", stdout);
    assert!(stdout.contains("[Open]") || stdout.contains("[OPEN]"), 
            "Should show PR state as Open, got: {}", stdout);
